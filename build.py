...
hbb_name = 'Anuvadini' + ('.exe' if windows else '')
...
def build_flutter_dmg(version, features):
    ...
    # copy dylib
    system2(
        "cp target/release/liblibanuvadini.dylib target/release/libanuvadini.dylib")
    os.chdir('flutter')
...
def build_flutter_arch_manjaro(version, features):
    ...
    os.chdir('flutter')
    system2('flutter build linux --release')
    system2(f'strip {flutter_build_dir}/lib/libanuvadini.so')
    os.chdir('../res')
...
def build_flutter_windows(version, features, skip_portable_pack):
    if not skip_cargo:
        system2(f'cargo build --features {features} --lib --release')
        if not os.path.exists("target/release/libanuvadini.dll"):
            if os.path.exists("target/release/anuvadini.dll"):
                shutil.copy2("target/release/anuvadini.dll", "target/release/libanuvadini.dll")
            else:
    ...
    system2(
        f'python3 ./generate.py -f ../../{flutter_build_dir_2} -o . -e ../../{flutter_build_dir_2}/Anuvadini.exe')
    os.chdir('../..')
...
def main():
    ...
    if windows:
        ...
        system2('mv target/release/anuvadini.exe target/release/Anuvadini.exe')
        pa = os.environ.get('P')
        if pa:
