...
lazy_static::lazy_static! {
    ...
    pub static ref APP_NAME: RwLock<String> = RwLock::new("Anuvadini".to_owned());
...
pub const LINK_DOCS_HOME: &str = "https://anuvadini.com/docs/en/";
pub const LINK_DOCS_X11_REQUIRED: &str = "https://anuvadini.com/docs/en/manual/linux/#x11-required";
pub const LINK_HEADLESS_LINUX_SUPPORT: &str =
    "https://github.com/anuvadini/anuvadini/wiki/Headless-Linux-Support";

lazy_static::lazy_static! {
    pub static ref HELPER_URL: HashMap<&'static str, &'static str> = HashMap::from([
        ("anuvadini docs home", LINK_DOCS_HOME),
        ("anuvadini docs x11-required", LINK_DOCS_X11_REQUIRED),
        ("anuvadini x11 headless", LINK_HEADLESS_LINUX_SUPPORT),
        ]);
}
...
impl Config {
    ...
    pub fn get_rendezvous_server() -> String {
        let mut rendezvous_server = Self::get_rendezvous_servers()
            .drain(..)
            .next()
            .unwrap_or_default();
    ...
    pub fn get_rendezvous_servers() -> Vec<String> {
        ...
        if let Ok(config) = CONFIG2.read() {
            if !config.rendezvous_server.is_empty() {
                servers.push(config.rendezvous_server.clone());
            }
        }
        if servers.is_empty() {
            servers.extend(RENDEZVOUS_SERVERS.iter().map(|x| x.to_string()));
        }
        servers
    }
...
pub fn option2bool(option: &str, value: &str) -> bool {
    if option == keys::OPTION_DIRECT_SERVER { return true; }
