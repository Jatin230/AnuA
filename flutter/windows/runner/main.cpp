...
typedef char** (*FUNC_ANUVADINI_CORE_MAIN)(int*);
typedef void (*FUNC_ANUVADINI_FREE_ARGS)( char**, int);
typedef int (*FUNC_ANUVADINI_GET_APP_NAME)(wchar_t*, int);
/// Note: `--server`, `--service` are already handled in [core_main.rs].
const std::vector<std::string> parameters_white_list = {"--install", "--cm", "--firewall-allow"};
...
{
  HINSTANCE hInstance = LoadLibraryA("libanuvadini.dll");
  if (!hInstance)
  {
    std::cout << "Failed to load libanuvadini.dll." << std::endl;
    return EXIT_FAILURE;
  }
  FUNC_ANUVADINI_CORE_MAIN anuvadini_core_main =
      (FUNC_ANUVADINI_CORE_MAIN)GetProcAddress(hInstance, "anuvadini_core_main_args");
  if (!anuvadini_core_main)
  {
    std::cout << "Failed to get anuvadini_core_main." << std::endl;
    return EXIT_FAILURE;
  }
  FUNC_ANUVADINI_FREE_ARGS free_c_args =
      (FUNC_ANUVADINI_FREE_ARGS)GetProcAddress(hInstance, "free_c_args");
  if (!free_c_args)
  {
  ...
  int args_len = 0;
  char** c_args = anuvadini_core_main(&args_len);
  if (!c_args)
  {
    std::string args_str = "";
    ...
    // std::cout << "Anuvadini [" << args_str << "], core returns false, exiting without launching Flutter app." << std::endl;
    return EXIT_SUCCESS;
  }
  std::vector<std::string> rust_args(c_args, c_args + args_len);
  free_c_args(c_args, args_len);

  std::wstring app_name = L"Anuvadini";
  FUNC_ANUVADINI_GET_APP_NAME get_anuvadini_app_name = (FUNC_ANUVADINI_GET_APP_NAME)GetProcAddress(hInstance, "get_anuvadini_app_name");
  if (get_anuvadini_app_name) {
    wchar_t app_name_buffer[512] = {0};
    if (get_anuvadini_app_name(app_name_buffer, 512) == 0) {
