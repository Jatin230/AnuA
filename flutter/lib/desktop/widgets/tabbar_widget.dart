...
class _DesktopTabState extends State<DesktopTab>
  ...
  void onWindowClose() async {
    mainWindowClose() async {
      // On Windows, exit the process so the app fully closes.
      // On other platforms (macOS/Linux), hide to allow background server.
      if (isWindows) {
        exit(0);
      } else {
    ...
    if (isMainWindow) {
      if (anuvadiniWinManager.getActiveWindows().contains(kMainWindowId)) {
        await anuvadiniWinManager.unregisterActiveWindow(kMainWindowId);
      }
  ...
  Widget build(BuildContext context) {
    return Column(children: [
      Obx(() {
        final hideWindowsMainTabStrip =
            isWindows &&
            (tabType == DesktopTabType.main || tabType == DesktopTabType.install);
        if (stateGlobal.showTabBar.isTrue &&
            !hideWindowsMainTabStrip &&
            !((kUseCompatibleUiMode || isWindows) && isHideSingleItem())) {
  ...
  bool isHideSingleItem() {
    ...
    return controller.tabType == DesktopTabType.main ||
...
class AddButton extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return ActionIcon(
        ...
        icon: IconFont.add,
        onTap: () => anuvadiniWinManager.call(
