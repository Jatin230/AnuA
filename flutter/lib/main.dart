...
Future<void> main(List<String> args) async {
  ...
  if (args.isNotEmpty && args.first == 'multi_window') {
    ...
    // Keep native caption buttons visible on Windows
    if (!isMacOS) {
...
Future<void> initEnv(String appType) async {
  ...
  await initGlobalFFI();
  // Repair an earlier build that wrote public servers as
  // custom servers. Leaving these values in app data makes public routing look
  // like a custom deployment and can break connection fallback behavior.
  try {
    final fixed =
        await bind.mainGetLocalOption(key: 'fixed-public-server-config-v2');
    if (fixed != 'true') {
      final idServer =
      ...
      await bind.mainSetLocalOption(
          key: 'fixed-public-server-config-v2', value: 'true');
    }
  } catch (e) {
    debugPrint("Failed to repair public server config: $e");
  }
...
void _runApp(
  ...
  runApp(RefreshWrapper(
    builder: (context) => GetMaterialApp(
      ...
      title: title,
      theme: AnuvadiniThemes.getThemeData(bind.mainGetLocalOption(key: kCommConfKeyTheme)),
...
_registerEventHandler() {
  if (isDesktop && desktopType != DesktopType.main) {
    platformFFI.registerEventHandler('theme', 'theme', (evt) async {
      ...
      if (dark != null) {
        await MyTheme.changeThemeString(dark);
      }
    });
