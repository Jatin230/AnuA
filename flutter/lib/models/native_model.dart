...
class PlatformFFI {
  ...
  AnuvadiniImpl get ffiBind => _ffiBind;
  ...
  Future<void> init(String appType) async {
    ...
    final dylib = isAndroid
        ? DynamicLibrary.open('libanuvadini.so')
        : isLinux
            ? DynamicLibrary.open('libanuvadini.so')
            : isWindows
                ? DynamicLibrary.open('libanuvadini.dll')
                :
                ...
                //
                // isMacOS? DynamicLibrary.open("liblibanuvadini.dylib") :
  ...
  void _startListenEvent(AnuvadiniImpl anuvadiniImpl) {
    final appType =
        _appType == kAppTypeDesktopRemote ? '$_appType,$kWindowId' : _appType;
    var sink = anuvadiniImpl.startGlobalEventStream(appType: appType);
    sink.listen((message) {
      () async {
        try {
