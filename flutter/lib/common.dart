...
class MyTheme {
  ...
  static Future<void> changeThemeString(String mode) async {
    Get.changeTheme(AnuvadiniThemes.getThemeData(mode));
    if (desktopType == DesktopType.main || isAndroid || isIOS || isWeb) {
      await bind.mainSetLocalOption(
          key: kCommConfKeyTheme, value: mode);
      if (!isWeb) await bind.mainChangeTheme(dark: mode == 'obsidian_night' || mode == 'cyber_proxy' || mode == 'legacy_console' ? 'dark' : 'light');
...
Future<void> windowOnTop(int? id) async {
  ...
  if (id == null) {
    ...
    await anuvadiniWinManager.registerActiveWindow(kWindowMainId);
  } else {
    WindowController.fromWindowId(id)
      ..focus()
      ..show();
    anuvadiniWinManager.call(WindowType.Main, kWindowEventShow, {"id": id});
  }
}
...
makeMobileActionsOverlayEntry(VoidCallback? onHide, {FFI? ffi}) {
  makeMobileActions(BuildContext context, double s) {
    ...
    const double overlayW = 45; // Flipped width/height
    const double overlayH = 200;
    computeOverlayPosition() {
      final screenW = MediaQuery.of(context).size.width;
      final screenH = MediaQuery.of(context).size.height;
      final left = 20.0; // Pinned to left side now
      final top = (screenH - overlayH * scale) / 2; // Center Vertically
...
void msgBox(SessionID sessionId, String type, String title, String text,
  ...
  submit() {
    ...
    if (onSubmit != null) {
      ...
      // https://github.com/anuvadini/anuvadini/blob/5e9a31340b899822090a3731769ae79c6bf5f3e5/src/ui/common.tis#L263
...
Future<bool> restoreWindowPosition(WindowType type,
  ...
  if (bind
      .mainGetEnv(key: "DISABLE_ANUVADINI_RESTORE_WINDOW_POSITION")
      .isNotEmpty) {
    return false;
  }
  if (type != WindowType.Main && windowId == null) {
  ...
  if (lpos == null) {
    ...
    switch (type) {
      case WindowType.Main:
        ...
        // See https://github.com/anuvadini/anuvadini/blob/9b9276e7524523d7f667fefcd0694d981443df0e/flutter/macos/Runner/Base.lproj/MainMenu.xib#L333
      ...
      default:
        ...
        // https://github.com/anuvadini/anuvadini/blob/317639169359936f7f9f85ef445ec9774218772d/flutter/lib/utils/multi_window_manager.dart#L163
        break;
    }
    return true;
  }
      ...
      if (lpos.isMaximized == true) {
        ...
        if (isWindows) {
          if (storeSize) {
            ...
            // See known issue in https://github.com/anuvadini/anuvadini/pull/9840
  ...
  if (type != null && id != null) {
    switch (type) {
      case UriLinkType.remoteDesktop:
        Future.delayed(Duration.zero, () {
          anuvadiniWinManager.newRemoteDesktop(id!,
              password: password,
      ...
      case UriLinkType.fileTransfer:
        Future.delayed(Duration.zero, () {
          anuvadiniWinManager.newFileTransfer(id!,
              password: password, forceRelay: forceRelay);
        });
        break;
      case UriLinkType.viewCamera:
        Future.delayed(Duration.zero, () {
          anuvadiniWinManager.newViewCamera(id!,
              password: password, forceRelay: forceRelay);
        });
        break;
      case UriLinkType.portForward:
        Future.delayed(Duration.zero, () {
          anuvadiniWinManager.newPortForward(id!, false,
              password: password, forceRelay: forceRelay);
        });
        break;
      case UriLinkType.rdp:
        Future.delayed(Duration.zero, () {
          anuvadiniWinManager.newPortForward(id!, true,
              password: password, forceRelay: forceRelay);
        });
        break;
      case UriLinkType.terminal:
        Future.delayed(Duration.zero, () {
          anuvadiniWinManager.newTerminal(id!,
              password: password, forceRelay: forceRelay);
        });
        break;
      case UriLinkType.localtunnel:
        if (id != null) {
          connect(Get.context!, "tunnel:$id");
        }
        break;
    }
...
List<String>? urlLinkToCmdArgs(Uri uri) {
  ...
  if (uri.authority.isEmpty &&
    ...
    // anuvadini://<connect-id>
    // anuvadini://<connect-id>/r
    // anuvadini://<connect-id>/r@<server>
    command = '--connect';
    id = uri.authority;
    if (uri.path.length > 1) {
      id = id + uri.path;
...
connectMainDesktop(String id,
  ...
  if (isFileTransfer) {
    await anuvadiniWinManager.newFileTransfer(id,
        password: password,
        isSharedPassword: isSharedPassword,
        connToken: connToken,
        forceRelay: forceRelay);
  } else if (isViewCamera) {
    await anuvadiniWinManager.newViewCamera(id,
        password: password,
        isSharedPassword: isSharedPassword,
        connToken: connToken,
        forceRelay: forceRelay);
  } else if (isTcpTunneling || isRDP) {
    await anuvadiniWinManager.newPortForward(id, isRDP,
        password: password,
        isSharedPassword: isSharedPassword,
        connToken: connToken,
        forceRelay: forceRelay);
  } else if (isTerminal) {
    await anuvadiniWinManager.newTerminal(id,
        password: password,
        isSharedPassword: isSharedPassword,
        connToken: connToken,
        forceRelay: forceRelay);
...
connect(BuildContext context, String id,
  ...
  if (isDesktop) {
    if (desktopType == DesktopType.main) {
      ...
      await anuvadiniWinManager.call(WindowType.Main, kWindowConnect, {
        'id': id,
        'isFileTransfer': isFileTransfer,
        'isViewCamera': isViewCamera,
        'isTerminal': isTerminal,
...
Future<void> onActiveWindowChanged() async {
  print(
      "[MultiWindowHandler] active window changed: ${anuvadiniWinManager.getActiveWindows()}");
  if (anuvadiniWinManager.getActiveWindows().isEmpty) {
    // close all sub windows
    try {
      if (isLinux) {
        await Future.wait([
          saveWindowPosition(WindowType.Main),
          anuvadiniWinManager.closeAllSubWindows()
        ]);
      } else {
        await anuvadiniWinManager.closeAllSubWindows();
      }
    } catch (err) {
      debugPrintStack(label: "$err");
    } finally {
      debugPrint("Start closing Anuvadini...");
      ...
      if (isMacOS) {
        ...
        // 2024-11-11 11:41:11.546 Anuvadini[90272:2567686] Failed to create a FlutterPlatformMessageResponseHandle (2)
        // embedder.cc (2672): 'FlutterEngineSendPlatformMessage' returned 'kInvalidArguments'. Invalid engine handle.
        // 2024-11-11 11:41:11.565 Anuvadini[90272:2567686] Failed to send message to Flutter engine on channel 'flutter/lifecycle' (2).
        // ```
        periodic_immediate(
...
class ServerConfig {
  ...
  /// decode from shared string (from user shared or anuvadini-server generated)
  /// also see [encode]
...
const kMacOSPermChannel = MethodChannel("org.anuvadini.anuvadini/host");
...
Future<List<Rect>> getScreenListWayland() async {
  ...
  if (isMainDesktopWindow) {
    ...
    final screenList = await anuvadiniWinManager.call(
...
setNewConnectWindowFrame(int windowId, String peerId, int preSessionCount,
  ...
  if (screenRect == null) {
    ...
    // https://github.com/anuvadini/anuvadini/discussions/8825
...
Widget loadPowered(BuildContext context) {
  ...
  return MouseRegion(
    ...
    child: GestureDetector(
      onTap: () {
        launchUrl(Uri.parse('https://anuvadini.com'));
      },
      child: Opacity(
          opacity: 0.5,
          child: Text(
      ...
      children: [
        Align(
          ...
          style: TextStyle(
            color: Colors.red,
            fontSize:
                18, // https://github.com/anuvadini/anuvadini-server-pro/issues/261
            fontWeight: FontWeight.bold,
          ),
...
bool whitelistNotEmpty() {
  // https://anuvadini.com/docs/en/self-host/client-configuration/advanced-settings/#whitelist
...
// https://github.com/anuvadini/anuvadini/blob/ce1dac3b8613596b4d8ae981275f9335489eb935/flutter/lib/desktop/widgets/tabbar_widget.dart#L385
// https://github.com/anuvadini/anuvadini/blob/ce1dac3b8613596b4d8ae981275f9335489eb935/flutter/lib/desktop/widgets/tabbar_widget.dart#L399
//
// @platforms macos
...
String decode_http_response(http.Response resp) {
  try {
    // https://github.com/anuvadini/anuvadini-server-pro/discussions/758
