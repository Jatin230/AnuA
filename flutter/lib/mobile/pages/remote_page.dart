...
class _RemotePageState extends State<RemotePage> with WidgetsBindingObserver {
  ...
  void onSoftKeyboardChanged(bool visible) {
    if (!visible) {
      ...
      // https://github.com/anuvadini/anuvadini/discussions/11843#discussioncomment-13499698 - Virtual keyboard issue
      if (isIOS) {
        _iosKeyboardWorkaroundTimer?.cancel();
  ...
  Widget build(BuildContext context) {
    ...
    return WillPopScope(
      ...
      child: Scaffold(
          // workaround for https://github.com/anuvadini/anuvadini/issues/3131
...
class CursorPaint extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final adjust = c.getAdjustY();
    final cursorX = (m.x - hotx) * factor + c.x / s2;
    final cursorY = (m.y - hoty) * factor + (c.y + adjust) / s2;
    final hotspotX = m.x * factor + c.x / s2;
    final hotspotY = m.y * factor + (c.y + adjust) / s2;
    final forbidden = preForbiddenCursor.image != null &&
        !ffiModel.viewOnly &&
        !ffiModel.keyboard &&
        !ShowRemoteCursorState.find(id).value;
    return CustomPaint(
      ...
      foregroundPainter: _CursorAccentPainter(
        x: hotspotX,
        y: hotspotY,
        forbidden: forbidden,
      ),
...
class _CursorAccentPainter extends CustomPainter {
  final double x;
  final double y;
  final bool forbidden;

  _CursorAccentPainter({
    required this.x,
    required this.y,
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final color = forbidden ? const Color(0xFFDC2626) : const Color(0xFF14B8A6);
    final outer = Path()
      ...
      ..close();
    final inner = Path()
      ..moveTo(center.dx, center.dy - 4.2)
      ..lineTo(center.dx + 4.2, center.dy)
      ..lineTo(center.dx, center.dy + 4.2)
      ..lineTo(center.dx - 4.2, center.dy)
      ..close();
    canvas.drawPath(
      outer,
  ...
  @override
  bool shouldRepaint(covariant _CursorAccentPainter oldDelegate) {
    return oldDelegate.x != x ||
        oldDelegate.y != y ||
        oldDelegate.forbidden != forbidden;
  }
}
