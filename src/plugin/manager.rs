...
fn get_plugin_source_list() -> Vec<PluginSource> {
    ...
    // vec![PluginSource {
    //     name: "anuvadini".to_string(),
    //     url: "https://raw.githubusercontent.com/fufesou/anuvadini-plugins/main".to_string(),
...
fn elevate_install(
    ...
    crate::platform::elevate(args, "Anuvadini wants to install then plugin")
}
...
fn elevate_uninstall(plugin_id: &str) -> ResultType<bool> {
    crate::platform::elevate(
        ...
        "Anuvadini wants to uninstall the plugin",
    )
}
