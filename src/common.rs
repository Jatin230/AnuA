...
pub fn is_support_remote_print(ver: &str) -> bool {
    hbb_common::get_version_number(ver) >= hbb_common::get_version_number("1.3.0")
}
...
// Because the url is always `https://api.anuvadini.com/version/latest`.
#[tokio::main(flavor = "current_thread")]
pub async fn do_check_software_update() -> hbb_common::ResultType<()> {
    let (request, url) =
        hbb_common::version_check_request(hbb_common::VER_TYPE_ANUVADINI_CLIENT.to_string());
...
#[inline]
pub fn is_anuvadini() -> bool {
    hbb_common::config::APP_NAME.read().unwrap().eq("Anuvadini")
}
...
fn get_api_server_(api: String, custom: String) -> String {
    ...
    "https://admin.anuvadini.com".to_owned()
}

#[inline]
pub fn is_public(url: &str) -> bool {
    url.contains("anuvadini.com/") || url.ends_with("anuvadini.com")
}
...
pub async fn _get_key_old(sync: bool) -> String {
    #[cfg(windows)]
    if let Ok(lic) = crate::platform::windows::get_license_from_exe_name() {
        if !lic.key.is_empty() {
            return lic.key;
...
pub fn check_process(arg: &str, mut same_uid: bool) -> bool {
    ...
    for (_, p) in sys.processes().iter() {
        ...
        // on mac, p.cmd() get "/Applications/Anuvadini.app/Contents/MacOS/Anuvadini", "XPC_SERVICE_NAME=com.carriez.Anuvadini_server"
...
pub type AnuvadiniInterval = ThrottledInterval;

#[inline]
pub fn anuvadini_interval(i: Interval) -> ThrottledInterval {
...
mod tests {
    ...
    #[tokio::test]
    async fn test_Anuvadini_interval() {
        ...
        for maker in base_intervals.into_iter() {
            ...
            let mut timer = anuvadini_interval(maker());
    ...
    #[allow(non_snake_case)]
    #[tokio::test]
    async fn test_Anuvadini_interval_sleep() {
        let base_intervals = [interval_maker, interval_at_maker];
    ...
    #[test]
    fn test_is_public() {
        // Test URLs containing "anuvadini.com/"
        assert!(is_public("https://anuvadini.com/"));
        assert!(is_public("https://www.anuvadini.com/"));
        assert!(is_public("https://api.anuvadini.com/v1"));
        assert!(is_public("https://anuvadini.com/path"));

        // Test URLs ending with "anuvadini.com"
        assert!(is_public("anuvadini.com"));
        assert!(is_public("https://anuvadini.com"));
        assert!(is_public("http://www.anuvadini.com"));
        assert!(is_public("https://api.anuvadini.com"));
        ...
        assert!(!is_public("https://anuvadini.computer.com"));
        assert!(!is_public("anuvadini.comhello.com"));
    }
