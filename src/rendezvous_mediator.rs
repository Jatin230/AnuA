...
impl RendezvousMediator {
        ...
        loop {
            ...
            *SOLVING_PK_MISMATCH.lock().await = "".to_owned();
            ...
            if !MANUAL_RESTARTED.load(Ordering::SeqCst) {
                ...
                // https://github.com/anuvadini/anuvadini/issues/12233
                sleep(0.033).await;
            }
        }
    }
    ...
    pub async fn start_udp(server: ServerPtr, host: String) -> ResultType<()> {
        ...
        let (mut socket, mut addr) = match new_udp_for(&host, CONNECT_TIMEOUT).await {
            Ok(res) => res,
            Err(e) => {
                log::error!("Failed to create UDP socket for {}: {}", host, e);
                return Err(e);
            }
        };
        if let Some(local_addr) = socket.local_addr() {
            log::info!("UDP socket for {} bound to local address: {}", host, local_addr);
        }
        let mut rz = Self {
            addr: addr.clone(),
            host: host.clone(),
                    ...
                    match n {
                        Some(Ok((bytes, _))) => {
                            if let Ok(msg) = Message::parse_from_bytes(&bytes) {
                                if let Err(e) = rz.handle_resp(msg.union, Sink::Framed(&mut socket, &addr), &server, &mut update_latency).await {
                                    log::error!("Failed to handle response from {}: {}", host, e);
                                    return Err(e);
                                }
                            } else {
                ...
                _ = timer.tick() => {
                    ...
                    if timeout || (last_register_sent.is_none() && expired) {
                        if timeout {
                            fails += 1;
                            if fails >= MAX_FAILS2 {
                                log::warn!("Rendezvous connection to {} failed (fails: {})", host, fails);
                        ...
                        log::info!("Sending RegisterPeer to {}", rz.host);
    ...
    async fn handle_resp(
        ...
        match msg {
            ...
            Some(rendezvous_message::Union::RegisterPkResponse(rpr)) => {
                log::info!("Received RegisterPkResponse from {}", self.host);
                update_latency();
    ...
    async fn register_peer(&mut self, socket: Sink<'_>) -> ResultType<()> {
        ...
        if let Ok(bytes) = msg_out.write_to_bytes() {
            log::info!("Sending RegisterPeer ({} bytes) hex: {:02x?}", bytes.len(), bytes);
        }
        socket.send(&msg_out).await?;
        Ok(())
    }
    ...
    fn get_relay_server(&self, provided_by_rendezvous_server: String) -> String {
        let mut relay_server = "".to_string(); // Config::get_option("relay-server");
