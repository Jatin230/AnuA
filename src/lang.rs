...
pub fn translate_locale(name: String, locale: &str) -> String {
    ...
    let replace = |s: &&str| {
        ...
        if !crate::is_anuvadini() {
            if s.contains("Anuvadini")
                && !name.starts_with("upgrade_anuvadini_server_pro")
                && name != "powered_by_me"
            {
                let app_name = crate::get_app_name();
                if !app_name.contains("Anuvadini") {
                    s = s.replace("Anuvadini", &app_name);
                } else {
                    // https://github.com/anuvadini/anuvadini-server-pro/issues/845
                    // If app_name contains "Anuvadini" (e.g., "Anuvadini-Admin"), we need to avoid
                    // replacing "Anuvadini" within the already-substituted app_name, which would
                    // cause duplication like "Anuvadini-Admin" -> "Anuvadini-Admin-Admin".
                    //
