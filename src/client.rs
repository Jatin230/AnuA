...
pub const SCRAP_X11_REF_URL: &str = "https://anuvadini.com/docs/en/manual/linux/#x11-required";
...
impl Client {
        ...
        match Self::_start(peer, key, token, conn_type, interface.clone()).await {
            Err(err) => {
                ...
                if err_str.starts_with("Failed") {
                    bail!(err_str + " (PEER: " + peer + "): Please try later");
                } else {
                    bail!(err_str + " (PEER: " + peer + ")");
                }
            }
            Ok(x) => {
    ...
    if peer.starts_with("direct-tcp:") {
        // Strict filtering: only allow characters valid for IP/Port/IPv6
        let addr: String = peer[11..]
            .trim()
            .chars()
            .filter(|c| c.is_alphanumeric() || ".:[]_-".contains(*c))
            .collect();
        let addr = addr.replace("_port_", ":");
    ...
    fn check_clipboard(&mut self) {
        if CLIPBOARD_STATE.lock().unwrap().running {
            ...
            if let Some(urls) = check_clipboard_files(&mut self.ctx, ClipboardSide::Client, false) {
                if !urls.is_empty() {
                    #[cfg(target_os = "macos")]
                    if crate::clipboard::is_file_url_set_by_anuvadini(&urls) {
                        return;
                    }
    ...
    pub fn handle_peer_info(&mut self, pi: &PeerInfo) {
        ...
        #[cfg(feature = "flutter")]
        {
                ...
                && !self.password_source.is_personal_ab(&password)
            {
                ...
                m.insert("name".to_string(), "sync_peer_hash_password_to_personal_ab".to_string());
                m.insert("id".to_string(), self.id.clone());
                m.insert("hash".to_string(), hash);
                let evt = hbb_common::serde_json::to_string(&m).unwrap_or("".to_owned());
...
lazy_static::lazy_static! {
    static ref LOGIN_ERROR_MAP: Arc<HashMap<&'static str, LoginErrorMsgBox>> = {
        ...
        let map = HashMap::from([(LOGIN_SCREEN_WAYLAND, LoginErrorMsgBox{
            ...
            link: "https://anuvadini.com/docs/en/manual/linux/#login-screen",
            try_again: true,
...
pub trait Interface: Send + Clone + 'static + Sized {
    ...
    fn on_establish_connection_error(&self, err: String) {
        ...
        // deadline: https://github.com/anuvadini/anuvadini-server-pro/discussions/325, most likely comes from secure tcp timeout
        {
            relay_hint = true;
            if !received {
...
async fn hc_connection_(
    ...
    let mut timer = crate::anuvadini_interval(interval(crate::TIMER_OUT));
