...
pub struct Connection {
    ...
    timer: crate::AnuvadiniInterval,
    file_timer: crate::AnuvadiniInterval,
    ...
    pub(crate) last_supported_encoding: Option<hbb_common::message_proto::SupportedEncoding>,
    services_subed: bool,
    delayed_read_dir: Option<(String, bool)>,
    #[cfg(target_os = "macos")]
    pub(crate) retina: crate::platform::macos::Retina,
...
impl Connection {
    pub async fn start(
        ...
        let mut conn = Self {
            ...
            hash,
            read_jobs: Vec::new(),
            timer: crate::anuvadini_interval(time::interval(SEC30)),
            file_timer: crate::anuvadini_interval(time::interval(SEC30)),
            file_transfer: None,
            view_camera: false,
            terminal: false,
        ...
        let mut second_timer = crate::anuvadini_interval(time::interval(Duration::from_secs(1)));
            ...
            tokio::select! {
                ...
                _ = conn.file_timer.tick() => {
                    if !conn.read_jobs.is_empty() {
                        ...
                        conn.file_timer = crate::anuvadini_interval(time::interval_at(Instant::now() + SEC30, SEC30));
                    }
                }
                        ...
                        Some(message::Union::CursorPosition(_pos)) => {
                            #[cfg(not(any(target_os = "android", target_os = "ios")))]
                            {
                                if conn.follow_remote_cursor {
                                    conn.handle_cursor_switch_display(_pos.clone()).await;
                                }
                            }
                            #[cfg(target_os = "macos")]
                            if let Some(new_msg) = conn.retina.on_cursor_pos(&_pos, conn.display_idx) {
                                msg = Arc::new(new_msg);
                            }
                        }
    ...
    async fn send_logon_response_and_keep_alive(&mut self) -> bool {
        ...
        if self.require_2fa.is_some() && !self.is_recent_session(true) && !self.from_switch {
            self.handle_2fa_notification().await;
        ...
        self.finish_logon().await
    }

    async fn handle_2fa_notification(&self) {
    ...
    fn get_auth_conn_type(&self) -> (i32, AuthConnType) {
    ...
    fn build_peer_info(&self, _username: &mut String) -> PeerInfo {
        let mut pi = PeerInfo {
        ...
        pi.username = _username.clone();

        pi
    }


    async fn finish_logon(&mut self) -> bool {
        let mut _username = crate::platform::get_active_username();
        let mut pi = self.build_peer_info(&mut _username);
        let mut res = LoginResponse::new();

        if self.port_forward_socket.is_some() {
            res.set_peer_info(pi);
            self.send_logon_res(res).await;
            return true;
        }

        #[cfg(target_os = "linux")]
        if self.is_remote() {
            if let Some(err) = super::login_handler::check_linux_remote_error() {
                res.set_error(err);
                self.send_logon_res(res).await;
                return true;
            }
        ...
        let mut state = LogonState {
            pi,
            res,
            sub_service: false,
            wait_session_id_confirm: false,
        };

        #[cfg(windows)]
        if !self.terminal {
            self.handle_windows_specific_session(&mut state.pi, &mut state.wait_session_id_confirm);
        }

        if self.file_transfer.is_some() || self.terminal {
            state.res.set_peer_info(state.pi);
        } else if self.view_camera {
            super::login_handler::setup_camera_logon(self, &mut state).await;
        } else {
            super::login_handler::setup_remote_logon(self, &mut state).await;
        }

        let LogonState { res, sub_service, wait_session_id_confirm, .. } = state;
        self.send_logon_res(res).await;
        self.finish_post_logon_actions(sub_service, wait_session_id_confirm).await;
        true
    }
}

pub struct LogonState {
    pub(crate) pi: PeerInfo,
    pub(crate) res: LoginResponse,
    pub(crate) sub_service: bool,
    pub(crate) wait_session_id_confirm: bool,
}

impl Connection {

    async fn send_logon_res(&mut self, res: LoginResponse) {
        let mut msg_out = Message::new();
        msg_out.set_login_response(res);
        self.send(msg_out).await;
    }

    pub(crate) fn on_remote_authorized(&self) {
        self.update_codec_on_login();
    }
    async fn finish_post_logon_actions(&mut self, sub_service: bool, wait_session_id_confirm: bool) {
    ...
    pub(crate) fn update_codec_on_login(&self) {
        ...
        if let Some(p) = self.start_cm_ipc_para.take() {
            tokio::spawn(async move {
                ...
                .await
                {
                    ...
                    // https://github.com/anuvadini/anuvadini-server-pro/discussions/382#discussioncomment-10525725, cm may start failed
                    #[cfg(windows)]
                    if !crate::platform::is_prelogin()
    ...
    async fn on_message(&mut self, msg: Message) -> bool {
        ...
        if let Some(message::Union::LoginRequest(lr)) = msg.union {
            ...
            // https://github.com/anuvadini/anuvadini-server-pro/discussions/646
            ...
            match msg.union {
                ...
                Some(message::Union::KeyEvent(me)) => {
                    ...
                    if self.peer_keyboard_enabled() {
                        ...
                        // https://github.com/anuvadini/anuvadini/issues/8633
                        MOUSE_MOVE_TIME.store(get_time(), Ordering::SeqCst);
                        ...
                        // https://github.com/anuvadini/anuvadini/issues/6793
                    ...
                    if handle_fa {
                        ...
                        match fa.union {
                            ...
                            Some(file_action::Union::Receive(r)) => {
                                ...
                                // whenever got send/recv request, check peer version to ensure old version of anuvadini
        ...
        if self.keyboard {
            if let Ok(displays) = display_service::try_get_displays() {
                ...
                if let Some(display) = displays.get(display_idx) {
                    ...
                    #[cfg(windows)]
                    if let Some(_ok) =
                        virtual_display_manager::anuvadini_idd::change_resolution_if_is_virtual_display(
                            &name,
                            r.width as _,
                            r.height as _,
                        )
    ...
    async fn process_new_read_job(&mut self, mut job: fs::TransferJob, path: String) {
        ...
        self.file_timer = crate::anuvadini_interval(time::interval(MILLI1));
    ...
    #[inline]
    pub(crate) async fn send(&mut self, msg: Message) {
...
pub(crate) fn try_activate_screen() {
    #[cfg(windows)]
    std::thread::spawn(|| {
