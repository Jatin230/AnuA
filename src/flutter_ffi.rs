...
pub fn start_localtunnel_host(port: u16) {
    hbb_common::tokio::spawn(async move {
        match crate::localtunnel::LocaltunnelHost::start(port).await {
            Ok(url) => {
                let mut m = HashMap::new();
                m.insert("name".to_string(), "on_localtunnel_ready".to_string());
                m.insert("url".to_string(), url);
                let event = hbb_common::serde_json::to_string(&m).unwrap_or("".to_owned());
                crate::flutter::push_global_event(crate::flutter::APP_TYPE_MAIN, event);
            }
            Err(e) => {
                let mut m = HashMap::new();
                m.insert("name".to_string(), "on_localtunnel_error".to_string());
...
pub fn main_set_input_source(_session_id: SessionID, _value: String) {
    #[cfg(not(any(target_os = "android", target_os = "ios")))]
    {
        change_input_source(_session_id, _value);
        if let Some(session) = sessions::get_session_by_session_id(&_session_id) {
            try_sync_peer_option(&session, &_session_id, "input_source", None);
        }
    }
}
    ...
    if key == "is-printer-installed" {
        ...
        if key.starts_with("download-data-") {
            ...
            return match (
                ...
                (Ok(true), false) => format!("anuvadini-{_version}-x86_64.msi"),
                (Ok(true), true) | (Ok(false), _) => format!("anuvadini-{_version}-x86_64.exe"),
                (Err(e), _) => {
            ...
            #[cfg(target_os = "macos")]
            {
                return if cfg!(target_arch = "x86_64") {
                    format!("anuvadini-{_version}-x86_64.dmg")
                } else if cfg!(target_arch = "aarch64") {
                    format!("anuvadini-{_version}-aarch64.dmg")
                } else {
