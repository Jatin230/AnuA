// https://github.com/anuvadini/anuvadini/discussions/6444#discussioncomment-9010062
...
//*************************************************************
//
//  DeleteAnuvadiniTestCertsW_SingleHive()
//
//  Purpose:    Deletes Anuvadini Test certificates and wrong key stores
//
...
BOOL DeleteAnuvadiniTestCertsW_SingleHive(HKEY RootKey, LPWSTR Prefix = NULL) {
...
extern "C" void DeleteAnuvadiniTestCertsW() {
	// Current user
	std::wcout << "*** Current User" << std::endl;
	DeleteAnuvadiniTestCertsW_SingleHive(HKEY_CURRENT_USER);

	// Local machine (requires admin rights)
	std::wcout << "*** Local Machine" << std::endl;
	DeleteAnuvadiniTestCertsW_SingleHive(HKEY_LOCAL_MACHINE);
	...
	for (DWORD Index = 0; ; Index++) {
		...
		DeleteAnuvadiniTestCertsW_SingleHive(HKEY_USERS, SubKeyName);
	}
